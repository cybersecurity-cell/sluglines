...
- **Baseline N:** 34 test files / 996 assertion call sites, all passing (recounted 2026-08-22, D-35; raised by `tests/scheduled-jobs.test.mjs` in D-46 and the bypass assertions in D-47). This line is machine-checked — `tests/baseline-n.test.mjs` re-measures the repo and fails if it and this header disagree, so `N` cannot drift again without the change that moved it saying so. Supersedes D-25's "12 files / 99 assertions", which was stale by more than 2×.
