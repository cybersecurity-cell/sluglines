...
| Artifact | Location |
|---|---|
| Canonical repo (`Sluglines-AI`) | `github.com/cybersecurity-cell/Sluglines-AI`; local checkout `C:\Users\kalai\Projects\Sluglines-AI` |
| Content repo (`sluglines`) | `github.com/cybersecurity-cell/sluglines`; branches `main`, `codex/phase-3-4`; draft PR #1 |
...
| This document | `C:\Users\kalai\Projects\Temp\Sluglines\2026-08-14-consolidated-architecture.md` until it lands at `docs/` in the canonical repo |
...
### 3.4 `Sluglines-AI` — the canonical build
Next.js 16 / React 19 / TS / Zod 4 / Tailwind 4 / Supabase (Postgres + Auth + Realtime + pg_cron) / web-push / Vitest against a live database / `@anthropic-ai/sdk`. Repo evidence shows deployment config for `sluglines-ai.vercel.app` and a CI workflow on push (live status: UNVERIFIED until P0). Security docs exist **in the canonical repo** and are incorporated here by reference as normative: `docs/security/threat-model.md` (threats T1–T10 — the T-numbers cited in this document resolve there), `docs/security/authorization-matrix.md`, `docs/security/data-classification.md` (this is "the retention schedule" this document cites; P0 verifies it covers every table named in §8 and extends it where it does not). Known gaps: AI layer never run against a live model; SMS fallback is a stub; savings estimates are a flat constant; voice is browser-only Web Speech API.
...
- **Repo identity:** §15 Q1 (which repo *name* survives) is open; if it resolves to "transplant into the `sluglines` repo", the §12 prompts' repo URL changes but nothing else in this document does. Until Q1 is decided, §12 targets `github.com/cybersecurity-cell/Sluglines-AI`.
...
1. **Repo topology** — merge `sluglines` into `Sluglines-AI` or transplant to keep the `sluglines` name. Until decided, §12 targets `Sluglines-AI`; the decision changes one line in DECISIONS.md.
...
## 18. Rev. 5 changelog (panel resolutions)
