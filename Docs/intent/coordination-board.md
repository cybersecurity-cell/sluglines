...
- **Undo belongs to the owner.** A poster cancels their offer; a rider releases their ACTIVE seat or withdraws their own CONFIRMED seat before ARRIVING. A confirmed withdrawal reopens through `RELEASED` and does not automatically promote anyone. The poster may deliberately trigger manual FIFO promotion of the oldest ACTIVE waitlist entry for that offer; an empty queue leaves the seat open. Both are server actions with keys derived from (offer, revision) so a double tap replays (D-90, D-96). *Rejected:* a fourteenth route (§8 M3 names thirteen); a rider cancelling the whole offer (D-83 closed that); automatic waitlist promotion on withdrawal.
