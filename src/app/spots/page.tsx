...
function SpotItem({ name, lat, lng, active }: Spot) {
      const liClass = active
        ? 'flex items-center justify-between py-2 border-b border-slate-100 last:border-0'
              : 'flex items-center justify-between py-2 border-b border-slate-100 last:border-0 opacity-50'
      const dotClass = active ? 'w-2 h-2 rounded-full shrink-0 bg-green-500' : 'w-2 h-2 rounded-full shrink-0 bg-slate-300'
      return (
              <li className={liClass}>
                        <div className="flex items-center gap-2">
                                <span className={dotClass}></span>span>
...
export default function SpotsPage() {
      return (
              <div className="max-w-6xl mx-auto px-4 py-10">
                  ...
                  {/* I-395 / I-95 Corridor */}
                    <section className="mb-12">
                            ...
                            <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
                                      <div>
                                                  <h3 className="text-lg font-bold text-slate-800 mb-4">Morning Sluglines (Suburbs to DC)</h3>h3>
                                                  <div className="space-y-4">
                                                                <SpotGroup county="Fairfax" spots={MORNING_395_FAIRFAX} />
                                                                <SpotGroup county="Prince William" spots={MORNING_395_PRINCE_WILLIAM} />
                                                                <SpotGroup county="Stafford" spots={MORNING_395_STAFFORD} />
                                                  </div>div>
                                      </div>div>
                                      <div>
                                                  <h3 className="text-lg font-bold text-slate-800 mb-4">Afternoon Sluglines (DC to Suburbs)</h3>h3>
                                                  <div className="space-y-4">
                                                                <SpotGroup county="Fairfax" spots={AFTERNOON_395_FAIRFAX} />
                  ...
                  {/* Legend */}
                    <div className="bg-slate-50 rounded-xl p-4 flex flex-wrap items-center gap-6 text-sm text-slate-600">
                            <div className="flex items-center gap-2">
                                      <span className="w-3 h-3 rounded-full bg-green-500 inline-block"></span>span>
                                      Active spot
                            </div>div>
                            <div className="flex items-center gap-2">
                                      <span className="w-3 h-3 rounded-full bg-slate-300 inline-block"></span>span>
                                      Inactive spot
                            </div>div>
                            <div className="flex items-center gap-2">
